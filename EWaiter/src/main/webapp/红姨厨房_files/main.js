        var NameLogo;
        var gdata;
        try { gdata = JSON.parse(localStorage.getItem('gdata')); } catch (ex) { gdata = null };
        if (gdata == null) { gdata = {}; };
        //用户信息
        if (gdata.userinfo==undefined) { gdata.userinfo = {} };
        if( gdata.takeinfo==undefined){gdata.takeinfo={}};
        if (gdata.addresslist==undefined) { gdata.addresslist = [] };
        ...
        function loadLazy(targetId,defaultImg){
            var blazy = new Blazy({container:'#'+targetId, 
...
//                        $user.mark = 0;//0代表游客，1代表会员，2代表微信，3代表微博
        ...
        function submitOrder(){
                ...
                //var sub_data = "ML_GUID="+sub_ML_GUID+"&TI_Code="+sub_TI_Code+"&PD_Type="+sub_PD_Type+"&T_Version="+sub_T_Version+"&DB_Remark="+sub_DB_Remark+"&xml="+ encodeURIComponent(sub_xml);
                //decimal r_id, string ml_guid, string ti_code, int pd_type, string t_version, string db_remark, string xml
                ...
                function takeoutOrder(){
                    ...
                    //var sub_data02="r_id="+R_ID+"&ml_guid="+sub_ML_GUID+"&ti_code="+sub_TI_Code+"&pd_type="+sub_PD_Type+"&t_version="+sub_T_Version+"&db_remark="+sub_DB_Remark+"&xml="+encodeURIComponent(sub_xml);
                    //获取菜品分类
                    ...
                    //add(decimal r_id, string ml_guid, int pd_type,decimal charge,decimal sum, 
                    //DateTime b_time, string b_address, short b_peoplenum, string b_invoiceunit, 
        ...
        function toCartePage(){
            ...
            new iScroll("carte_dish_scroller",{ hScrollbar: false, vScrollbar: false, lockDirection: true,hScroll:false,vScroll:true,useTransition: true,click:true});
        ...
        function refreshCart(targetId){
		    setTimeout(function(){
                    ...
                    for(var i = 0 ; i < $order.size(); i++){
                        ...
                        if(add_flag)
                        {
                            _tmp.push({ctid:dishItemTemp.ctid,num:dishItemTemp.num});
                        };
...
//                         };
                    ...
                    //更新左侧菜单
//                    $("#"+targetId).html("共计"+total_number+"个菜,&nbsp<span class='price'>￥"+total_price.toFixed(2)+"</span>");
//                    $("#carte_info").html("共计"+total_number+"个菜,&nbsp<span class='price'>￥"+total_price.toFixed(2)+"</span>");
        ...
        function subOrder(target){
            ...
            for(var i = 0 ; i < $order.size(); i++){
                ...
                if(dishItemTemp.id == data_id){
                    ...
                    if(dishItemTemp.num == 0){$order.remove(i);}
                    break;
                }
            }
            refreshCart("dish_info");
        ...
        function bindSearchOpeAction(target){
             $(target).find('.sub').bind("click",function(e){
            ...
            $(target).find('.add').bind("click",function(e){
                var $click_time = e["timeStamp"];
        ...
        function bindOpeAction(target){
            $(target).find('.dish_add').bind("click",function(e){
                e.stopPropagation();
                var $click_time = e["timeStamp"];
                if($click_time-$last_click_time < 400) return;
                $last_click_time = $click_time;
            ...
            $(target).find('.sub').bind("click",function(e,igore){
                e.stopPropagation();
                var $click_time = e["timeStamp"];
            ...
            $(target).find('.number').bind("click",function(e){
                e.stopPropagation();
          
            });
            $(target).find('.add').bind("click",function(e,igore){
                e.stopPropagation();
                var $click_time = e["timeStamp"];
                if(igore == null && $click_time-$last_click_time < 400) return;
                $last_click_time = $click_time;
        ...
        function readDishItemData(target_current){
            ...
            var dish_data ={id:target.attr("data-id"), img:target.attr('data-img'),name:target.attr('data-name'),price:target.attr('data-price'),
                                vip:target.attr("data-vip-price"), num:target.children('.number').html(),ctid:target.attr('d-ctid'),mark:target.attr('data-remark')};
            return dish_data;
        }

        function bindDetailAction(target){
            //$(target).find('.dish_image').bind("click",function(e){
            $(target).find('.dish_item').bind("click",function(e){
               ...
               // document.getElementById("dish_details").style.webkitTransform = "translate3d(0%, 0, 0)";
                $("#dish_details").show();
                $("#dish_details").html(tmpl("tmpl_dish_detail_one",_data));
                $("#close_dish").bind("click",function(){
                    //document.getElementById("dish_details").style.webkitTransform = "translate3d(0, 1000px, 0)";
                    $("#dish_details").hide();
                });
                $("#mask_dish").bind("click",function(){
                   // document.getElementById("dish_details").style.webkitTransform = "translate3d(0, 1000px, 0)";
                     $("#dish_details").hide();
                });
                ...
                $("#dish_details .dish_add").bind("click",function(e){
                    e.stopPropagation();
                ...
                $("#dish_details .sub").bind("click",function(e){
                    e.stopPropagation();
                ...
                $("#dish_details .add").bind("click",function(e){
                    e.stopPropagation();
               ...
               $dish_detail_scroll=new iScroll("dish_detail_scroll",{hScrollbar: false, vScrollbar: false, lockDirection: true,hScroll:false,vScroll:true,useTransition: true,click:true});
                ...
                var imgscroll = new ImageScroll("image-scroll", test_data, function (o) {
                    //{ id: 1, img: "", name:"", price:"",vip:0,num:1,ctid:1,mark:"备注"}
            ...
            if($dish_load == false){
                ...
                jAjax({
                    ...
                    success: function(data) {
                        ...
                        $dish_scroller = new iScroll("dish_list_scroller",{hScrollbar: false, vScrollbar: false, lockDirection: true,
                            hScroll:false,vScroll:true,useTransition: true,click:true,
                            onTouchEnd:function(){
                        ...
                        $("#dish_search").bind("click",function(){showSearch();});
        ...
        function initDishList(){
            if($menu_load == false){
                 ...
                 jAjax({
                    ...
                    success: function(data) {
                       ...
                       // var obj=data;
                        ...
                        $dish_scroller = new iScroll("dish_category_scroller",{hScrollbar: false, vScrollbar: false, lockDirection: true,hScroll:false,vScroll:true,useTransition: true,click:true});
                        loadCarteAll();
                        bindSearchContent();
                        $("#dish_category_scroller li").bind("click",function(){
                                ...
                                setTimeout(function(){$blazy_control.validate();},500);
                                }catch(ex){alert(ex);}
                ...
                document.getElementById("slider_dish").style.webkitTransform = "translate3d(0,0,0)";
                setTimeout(function(){$("#loading_img").removeClass("rotate_animation");},500);
            }
        }
        
        function bindToSliderMenu() {
            $("#slider_menu_back").bind("click", function() {
                document.getElementById("slider_menu").style.webkitTransform = "translate3d(100%, 0, 0)";
            });
            /*选择桌台*/
            $("#open_slider_table").bind("click", function(e) {
                var $click_time = e["timeStamp"];
                if($click_time-$last_click_time < 400) return;
                $last_click_time = $click_time;
                document.getElementById("slider_table_Full").style.display="block";
        ...
        function init_shop() {
            ...
            //构造人数信息
             ...
             $person_scroller = new iScroll("person_category",{hScrollbar: false, vScrollbar: false, lockDirection: true,hScroll:false,vScroll:true,useTransition: true,click:true});
             $("#cancel_table").bind("click",function(){
                 document.getElementById("slider_table_Full").style.display="none";
                 $("#slider_table").css({ "-webkit-transform": "translate3d(0,100%, 0)"});
             });
             $("#cancel_person").bind("click",function(){
                $("#slider_person").css({ "-webkit-transform": "translate3d(0,100%, 0)"});
             });

             $("#person_category").find("li").bind("click",function(){
...
//                    alert(JSON.stringify(data));
            ...
            $('#select_table,#select_table2').bind("click",function(){
                document.getElementById("slider_table_Full").style.display="block";
                document.getElementById("slider_table").style.webkitTransform = "translate3d(0,0, 0)";
            });
            jAjax({
                type: "post",
                url: "/1/basic.asmx/tables",
                data:"r_id="+R_ID,
                showLoading: true,
                success: function(data) {
                    var obj = eval('(' + data + ')');
                    ...
                    $table_scroller = new iScroll("table_category",{hScrollbar: false, vScrollbar: false, lockDirection: true,hScroll:false,vScroll:true,useTransition: true,click:true});
                    $("#table_category").find("li").bind("click",function(e){
                         var $click_time = e["timeStamp"];
                        if($click_time-$last_click_time < 400) return;
                        $last_click_time = $click_time;
        ...
        function bindToDishAction() {
            $("#loading_img").bind("click", function() {
        ...
        function bindHideSearchAction(){
            $("#search_content_finish").bind("click", function() {
            ...
            $timer_search = setTimeout(function(){
                ...
                if(searchResult.size() >0){
                    ...
                    new iScroll("search_result",
                            {hScrollbar: false, vScrollbar: false, lockDirection: true,hScroll:false,vScroll:true,useTransition: true,click:true,
                             onTouchEnd:function(){
                                $blazy.validate();
			                 },
                             onScrollEnd:function(){
                ...
                $("#search_dish_noresult").bind("click",function(e){
                    e.stopPropagation();
                });
                $("#search_dish_scroller").bind("click",function(e){
                    e.stopPropagation();
                });
        ...
        $(document).ready(function() {// return;
            $qaPage_scroll = new iScroll("mylist",{hScrollbar: false, vScrollbar: false, lockDirection: true,hScroll:false,vScroll:true,useTransition: true,click:true});
...
//            });
            ...
            $("#index_waimai").click(function(){
                ...
                $takeaway_list_scroll = new iScroll("takeawaymain",{hScrollbar: false, vScrollbar: false, lockDirection: true,hScroll:false,vScroll:true,useTransition: true,click:true});
                            ...
                            if(obj.result==0)
                            {
                                ...
                                if(gdata.addresslist.length>0)
                                {
                                   ...
                                   $("#mylist_scroll .item").click(function(){
                                       ...
                                       $("#takeaway_list").append(  tmpl("takeaway_model5",{name:_name,img:gdata.userinfo.M_Image,mark:"name"}));
                                       $("#takeaway_list").append(  tmpl("takeaway_model5",{name:_phone,img:gdata.userinfo.M_Image,mark:"phone"}));
                                       $("#takeaway_list").append(  tmpl("takeaway_model5",{name:_address,img:gdata.userinfo.M_Image,mark:"address"}));
                                       $("#takeaway_list").attr("mark","end");
                                       gdata.takeinfo.name=_name;
                                       gdata.takeinfo.phone=_phone;
                                       gdata.takeinfo.address=_address;
                                       $("#takeaway_list").append(tmpl("takeaway_model4",{img:"/dish/"+gdata.shop_config.RestaurantSign+"/restaurant.png"}));
                                       $takeaway_list_scroll.refresh();
                                       $takeaway_list_scroll.scrollToElement($("#takeaway_list").children(":last")[0],500);
                                       $("#mylist").slideUp(500);
                                       $("[a-click]").unbind("click").bind("click",function(){
                                            if($shop_load && $table_load){
                                                initDishList();
                                                refreshCart("dish_info");
                                                dineType=1;//外卖
            ...
            $("#messagebtn").click(function(){
                ...
                if(chatFlag)
                {
                    ...
                    if(chatFlag=="1")
                    {
                        ...
                        $("#takeaway_list").append(  tmpl("takeaway_model5",{name:str,img:gdata.userinfo.M_Image,mark:"name"}));
                    ...
                    }else if(chatFlag=="2")
                    {
                        ...
                        {
                            _goFlag=false ;
                            $("#takeaway_list").append(  tmpl("takeaway_model6",{message:"手机号码输入有误。",img:"/dish/"+gdata.shop_config.RestaurantSign+"/restaurant.png"}));
                            inputObj.val('');
                            $takeaway_list_scroll.refresh();
                            $takeaway_list_scroll.scrollToElement($("#takeaway_list").children(":last")[0],500);
                        ...
                        }else
                        {
                            ...
                            $("#takeaway_list").append(  tmpl("takeaway_model5",{name:str,img:gdata.userinfo.M_Image,mark:"phone"}));
                            gdata.takeinfo.phone=str;
                        }
                    }else
                    {
                        $("#takeaway_list").append(  tmpl("takeaway_model5",{name:str,img:gdata.userinfo.M_Image,mark:"address"}));
                    ...
                    if(_goFlag)
                    {
                        ...
                        {
                            $("#takeaway_list").append(tmpl("takeaway_model2",{"name":"名字",img:"/dish/"+gdata.shop_config.RestaurantSign+"/restaurant.png"}));
                        }else if(takeaway_mark=="phone"){
                        
                            $("#takeaway_list").append(tmpl("takeaway_model2",{name:"电话",img:"/dish/"+gdata.shop_config.RestaurantSign+"/restaurant.png"}));
                        }else if(takeaway_mark=="address"){
                            $("#takeaway_list").append(tmpl("takeaway_model2",{name:"地址",img:"/dish/"+gdata.shop_config.RestaurantSign+"/restaurant.png"}));
                        }else
                        {
                           $("#takeaway_list").append(tmpl("takeaway_model4",{img:"/dish/"+gdata.shop_config.RestaurantSign+"/restaurant.png"}));
                           $("[a-click]").unbind("click").bind("click",function(){
                ...
                }else
                {
                    ...
                    if(takeaway_mark=="name")
                    {
                       ...
                       $("#takeaway_list").append(tmpl("takeaway_model2",{"name":"电话",img:"/dish/"+gdata.shop_config.RestaurantSign+"/restaurant.png"}));
                        ...
                        if(gdata.ML_GUID)
                        {
                            if(gdata.addresslist.length>=10){return ;}
                            var newAddress=true ;
...
//                                        data: "ml_guid=" + gdata.ML_GUID + "&address=" + encodeURIComponent(gdata.takeinfo.address) + "&contact=" + encodeURIComponent(gdata.takeinfo.name) + "&phone=" + gdata.takeinfo.phone + "&is_default=false",
//                                        success: function (data) {
//                                            var obj2 = eval('(' + data + ')');
//                                            if(obj2.result==0){
//                                                gdata.addresslist = obj2.addresses;
//                                                showdialog(1,"保存成功!");
//                                                setTimeout(function(){
//                                                    $("#mylist_scroll").html(tmpl("temp_myaddresslist2",gdata.addresslist));
//                                                    $mylist_scroll.refresh();
...
//                                });
                    ...
                    }else
                    {
                       ...
                       $("#takeaway_list").append(tmpl("takeaway_model3",{img:"/dish/"+gdata.shop_config.RestaurantSign+"/restaurant.png"}));
                       $("[a-click]").unbind("click").bind("click",function(){
                            if($shop_load && $table_load){
                                initDishList();
                                refreshCart("dish_info");
           ...
           $(".myAddress .item2").live("click",function(){
                ...
                {
                    //$(this).css({"-webkit-transform":"translate3d(-60px, 0px, 0px)"});
                    $(this).stop().animate({left:"-60px"});
                    $(this).attr("del",true)
                }
           });

           $("#takeawaymain .message").live("click",function(){
           ...
           $("#takeawaymain .tip").live("click",function(){
               var _mark=$(this).parents(".item2").attr("mark");
               $(this).hide();
               if(_mark=="name")
               {
          ...
          //delete(string ml_guid, decimal id)
           $(".myAddress .item2 .del").live("click",function(){
